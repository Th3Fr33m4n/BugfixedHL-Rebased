...
void CDisplacerBall::ExplodeThink( void )
{
	...
	pev->dmg = gSkillData.plrDmgDisplacer;
	...
	RadiusDamage(pev->origin, pev, pevOwner, gSkillData.plrDmgDisplacer, gSkillData.plrDisplacerRadius, CLASS_NONE, DMG_ALWAYSGIB | DMG_BLAST);
...
{
	if (!CanFireDisplacer(DISPLACER_SECONDARY_USAGE))
	{
		m_iPlayEmptySound = 1;
		PlayEmptySound();
...
{
	if (!CanFireDisplacer(DISPLACER_PRIMARY_USAGE))
	{
		m_iPlayEmptySound = 1;
		PlayEmptySound();
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.5f;
...
void CDisplacer::Teleport( void )
{
	...
	if( pTarget && /*HACK*/( tmp != g_vecZero/*HACK*/ ) )
	{
