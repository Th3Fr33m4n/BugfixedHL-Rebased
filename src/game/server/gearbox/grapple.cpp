	...
	if (m_pTip)
	{
		if (m_pTip->IsStuck())
		{
			...
			case GRAPPLE_SMALL:
				...
				{
					pTarget->pev->velocity = pTarget->pev->velocity.Normalized() * 450.0;
				}
			...
			case GRAPPLE_FIXED:
				...
				{
					m_pPlayer->pev->velocity = m_pPlayer->pev->velocity.Normalized() * 450.0;
				...
				if (m_pTip)
				{
					...
					if (bValidTarget)
					{
						if (m_flDamageTime + 0.5 < gpGlobals->time)
						{
							...
							pHit->TraceAttack(m_pPlayer->pev, flDamage, gpGlobals->v_forward, &tr, DMG_SLASH | DMG_ALWAYSGIB);
...
void CBarnacleGrapple::CreateEffect(void)
{
	...
	Vector vecOrigin = m_pPlayer->GetGunPosition() + gpGlobals->v_forward * 16.0 + gpGlobals->v_right * 8.0 + gpGlobals->v_up * -8.0;
...
void CBarnacleGrapple::ItemPostFrame() {
	CBasePlayerWeapon::ItemPostFrame();
#ifndef CLIENT_DLL
	if (m_fireState != OFF && m_pTip && m_pTip->GetGrappleTarget()) {
		if (m_pTip->GetGrappleType() != GRAPPLE_FIXED && !m_pTip->GetGrappleTarget()->IsAlive()) {
			EndAttack();
		}
	}
#endif
